...
class ShareHandler : Object, PacketHandlerInterface {
    ...
    public signal void text_received (Device dev, string text);
    ...
    private void handle_file (Device dev, Packet pkt) {
        ...
        // Preserve file timestamps if provided
        int64 creation_time = 0;
        int64 last_modified = 0;
        bool should_open = false;

        if (pkt.body.has_member ("creationTime")) {
            creation_time = pkt.body.get_int_member ("creationTime");
        }
        if (pkt.body.has_member ("lastModified")) {
            last_modified = pkt.body.get_int_member ("lastModified");
        }
        if (pkt.body.has_member ("open")) {
            should_open = pkt.body.get_boolean_member ("open");
        }

        var dest_path = make_downloads_path (name);
        ...
        t.finished.connect (() => {
            // Apply file timestamps if provided
            if (last_modified > 0 || creation_time > 0) {
                var file = File.new_for_path (dest_path);
                try {
                    var info = file.query_info (FileAttribute.TIME_MODIFIED,
                                                FileQueryInfoFlags.NONE);
                    if (last_modified > 0) {
                        // KDE Connect sends ms since epoch, GLib uses seconds
                        info.set_attribute_uint64 (FileAttribute.TIME_MODIFIED,
                                                    (uint64) (last_modified / 1000));
                    }
                    if (creation_time > 0) {
                        info.set_attribute_uint64 (FileAttribute.TIME_CREATED,
                                                    (uint64) (creation_time / 1000));
                    }
                    file.set_attributes_from_info (info, FileQueryInfoFlags.NONE);
                } catch (Error e) {
                    warning ("failed to set file timestamps: %s", e.message);
                }
            }

            // Auto-open file if requested
            if (should_open) {
                debug ("auto-opening received file: %s", dest_path);
                try {
                    AppInfo.launch_default_for_uri ("file://" + dest_path, null);
                } catch (Error e) {
                    warning ("cannot launch application for %s: %s", dest_path, e.message);
                }
            }

            file_received (dev, dest_path);
        });
    ...
    private Packet make_file_share_packet (string filename, uint64 size,
                                            uint16 port, int64 creation_time = 0,
                                            int64 last_modified = 0) {
        ...
        if (creation_time > 0) {
            builder.set_member_name ("creationTime");
            builder.add_int_value (creation_time);
        }
        if (last_modified > 0) {
            builder.set_member_name ("lastModified");
            builder.add_int_value (last_modified);
        }
        builder.end_object ();
    ...
    public void share_file (Device dev, string path) throws Error {
        ...
        try {
            var fi = file.query_info (FileAttribute.STANDARD_SIZE + "," +
                                       ...
                                       FileQueryInfoFlags.NONE);
            size = fi.get_size ();
            creation_time = (int64) fi.get_attribute_uint64 (FileAttribute.TIME_CREATED) * 1000;
            last_modified = (int64) fi.get_attribute_uint64 (FileAttribute.TIME_MODIFIED) * 1000;
        } catch (Error e) {
            warning ("failed to obtain file info: %s", e.message);
            return;
        }
