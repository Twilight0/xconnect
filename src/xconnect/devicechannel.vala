...
public class DeviceChannel : Object {
    ...
    public DeviceChannel.from_connection (SocketConnection conn) {
        this._sock_conn = conn;
        this._socket = conn.get_socket ();
        this._isa = (InetSocketAddress) conn.get_remote_address ();
    ...
    public bool is_secure {
        get { return this._tls_conn != null; }
    }
    ...
    public async void send_identity_plain_text (Packet pkt) throws Error {
        string to_send = pkt.to_string () + "\n";
        debug ("send plain text identity data: %s", to_send);
        var output_stream = this._sock_conn.output_stream;
        yield output_stream.write_all_async (to_send.data, Priority.DEFAULT, null, null);
    }

    public async Packet? receive_identity_plain_text () throws Error {
        var input_stream = this._sock_conn.input_stream;
        var sb = new StringBuilder ();
        uint8 buf[1];
        while (true) {
            size_t bytes_read;
            yield input_stream.read_all_async (buf, Priority.DEFAULT, null, out bytes_read);
            if (bytes_read == 0) {
                break;
            }
            char c = (char) buf[0];
        ...
        string data = sb.str;
        debug ("received plain text identity line: %s", data);
        if (data.length == 0) {
            return null;
        }

        return Packet.new_from_data (data);
    }
