...
class NotificationHandler : Object, PacketHandlerInterface {
    ...
    /**
     * Emitted when a notification is received from the phone.
     * @dev: the device
     * @id: notification ID
     * @app: application name
     * @title: notification title/ticker
     * @icon_path: path to icon file (may be null)
     */
    public signal void notification_received (Device dev, string id,
                                               string app, string title,
                                               string? icon_path);

    /**
     * Emitted when a notification is cancelled/removed.
     */
    public signal void notification_cancelled (Device dev, string id);
    ...
    public void message (Device dev, Packet pkt) {
        ...
        if (pkt.payload != null) {
            try {
                GLib.FileUtils.open_tmp("xconnect-icon-XXXXXX", out tempfile);
            } catch (Error e) {
    ...
    public void handle_pkt (Device dev, Packet pkt, string? filename) {
        ...
        // Emit signal for D-Bus proxy
        notification_received (dev, id, app, ticker, filename);
